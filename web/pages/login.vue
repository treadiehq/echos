...
onMounted(async () => {
  // If user is already logged in (from state), redirect to home
  if (auth.user.value) {
    router.push('/');
    return;
  }

  // Only check auth if login was successful (from query param)
  // This avoids unnecessary 401 errors on the login page
  ...
  if (loginStatus === 'success') {
    ...
    auth.fetchCurrentUser().then(() => {
      // Use replace to remove query params from history
      router.replace('/');
    }).catch(() => {
    ...
    <div class="w-full max-w-md">
      ...
      <div class="bg-gray-500/5 border border-gray-500/10 rounded-2xl p-8 backdrop-blur-sm">
        ...
        <div v-if="submitted" class="text-center space-y-4">
          ...
          <h2 class="text-xl font-semibold text-white">Please check your email</h2>
